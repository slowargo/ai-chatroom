...
interface ConfigFile {
  ...
  server?: {
    ...
    /** Pre-computed admin password hash ("salt:hash" hex). Preferred over env plaintext bootstrap. */
...
export interface ServerConfig {
  ...
  /** Bind address. Defaults to 127.0.0.1 (loopback only). */
  host: string
  /** Scrypt hash of the admin password, stored as "salt:hash" (hex). Null means local mode (no admin password). */
...
/**
 * Hash a plaintext admin password with scrypt (salted), asynchronously to avoid
 ...
 */
export async function verifyAdminPassword(plaintext: string, stored: string): Promise<boolean> {
 ...
 * with NO access password and NO admin password configured. The explicit escape hatch is
...
export function checkSecureBind(cfg: {
  ...
  if (cfg.accessPassword || cfg.adminPasswordHash) return null
  return (
    `Refusing to start: binding to non-loopback host "${cfg.host}" with no authentication is unsafe.\n` +
    `Choose one of:\n` +
    `  1. Set an admin password (server.admin_password_hash / CHATROOM_ADMIN_PASSWORD_HASH / CHATROOM_ADMIN_PASSWORD) and/or an access password (CHATROOM_ACCESS_PASSWORD).\n` +
...
export async function loadServerConfig(): Promise<ServerConfig> {
  ...
  // Admin password resolution, in priority order:
  //   1. Pre-computed hash (config file or CHATROOM_ADMIN_PASSWORD_HASH env) — preferred, no startup hashing cost.
  //   2. Plaintext bootstrap (CHATROOM_ADMIN_PASSWORD env) — hashed once at startup, plaintext never stored.
  // admin password is env-pinned when it comes from an env var (pre-hashed or plaintext bootstrap),
  // not from config.json — in that case the admin UI must not offer to change it.
  const adminEnvPinned = !!(process.env.CHATROOM_ADMIN_PASSWORD_HASH || process.env.CHATROOM_ADMIN_PASSWORD)
  let adminPasswordHash: string | null =
    process.env.CHATROOM_ADMIN_PASSWORD_HASH ?? file.server?.admin_password_hash ?? null
  if (!adminPasswordHash) {
    const rawAdminPw = process.env.CHATROOM_ADMIN_PASSWORD
    if (rawAdminPw) {
      console.warn('[auth] CHATROOM_ADMIN_PASSWORD env var detected — hashing at startup. Store the hash (server.admin_password_hash or CHATROOM_ADMIN_PASSWORD_HASH) instead for production use.')
      adminPasswordHash = await hashAdminPassword(rawAdminPw)
    }
  }
 ...
 *
 * Holds the runtime-changeable knobs (admin password hash, access password, brake threshold) behind
...
export class RuntimeConfig {
  private _adminPasswordHash: string | null
  ...
  constructor(
    ...
    this._adminPasswordHash = cfg.adminPasswordHash
  ...
  /** Persist a new admin password hash. Caller must reject when envPinned.adminPasswordHash. */
  setAdminPasswordHash(hash: string): void {
    this._adminPasswordHash = hash
    this.persist({ admin_password_hash: hash })
  }
